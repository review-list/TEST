...
def ensure_required_source_files() -> None:
    missing: List[str] = []
    ...
    if missing:
        raise FileNotFoundError("必須ファイルが見つかりません: " + ", ".join(missing))

    style_css = ASSETS_SRC / "style.css"
    ...
    for name in [
        ...
        import_pattern = re.compile(
            rf'@import\s+(?:url\(\s*)?["\']?\.?/?{re.escape(name)}["\']?\s*\)?\s*;',
            re.IGNORECASE,
        )
        if not import_pattern.search(style_text):
            raise RuntimeError(f"style.css に {name} の @import がありません")
...
def normalize_work(w: Dict[str, Any]) -> Dict[str, Any]:
    ...
    ww["description"] = str(ww.get("description") or "").strip()
    ww["description_official"] = str(ww.get("description_official") or "").strip()
...
def make_work_desc(w: Dict[str, Any]) -> str:
    """作品詳細ページ用の保険説明。タイトルや価格の丸写し感を減らす。"""
    title = (w.get("title") or "").strip()
    actresses = [x for x in (w.get("actresses") or []) if str(x).strip()]
    maker = (w.get("maker") or "").strip()
    series = (w.get("series") or "").strip()
    genres = [x for x in (w.get("tags") or w.get("genres") or []) if str(x).strip()]
    parts = []
    if actresses:
        parts.append(f"出演は{actresses[0]}")
    if maker:
        parts.append(f"メーカーは{maker}")
    if series:
        parts.append(f"シリーズは{series}")
    if genres:
        parts.append(f"ジャンルは{genres[0]}")
    if w.get("_has_mov"):
        parts.append("サンプル動画あり")
    if w.get("_has_img"):
        parts.append("サンプル画像あり")
    if not parts:
        return clip_desc(f"{title}の作品詳細ページです。", 140)
    core = "、".join(parts[:4])
    return clip_desc(f"{title}の作品詳細ページです。{core}。", 140)
